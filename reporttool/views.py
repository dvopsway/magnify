...
def load_report(request, report_name):
    ...
    while i < param_count:
        if request.POST.get(request.session.get("param"+str(i))) == "":
            ...
            request.session['error_message'] = "Sorry, You can't leave any field blank"
            return HttpResponseRedirect(reverse("report:get_params"))
        else:
            formdata.append(request.POST.get(request.session.get("param"+str(i))))
            i += 1
    ...
    while i < param_count:
        raw_query = raw_query.replace(request.session.get("param"+str(i)),formdata[i])
        i += 1
    ...
    try:
        if db_details.database_type == "mysql":
		    cursor = get_mysql_response(db_details.database_server , db_details.database_name , db_details.database_username , db_details.database_password , raw_query)
        else:
		    cursor = get_mssql_response(db_details.database_server , db_details.database_name , db_details.database_username , db_details.database_password , raw_query)
    except:
        request.session['valid_form'] = "yes"
        request.session['valid_report'] = "no"
        request.session['error_message'] = "Running SQL query is throwing exception"
        return HttpResponseRedirect(reverse("report:get_params"))
